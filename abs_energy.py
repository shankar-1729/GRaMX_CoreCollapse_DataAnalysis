...
plt.title("{}::abs_energy".format(var_name))
plt.ylim(-1e-8, 2e-6)
...
for output_number in range(0, latest_copied_output+1):     
    ...
    if style:  
        plt.scatter(data["2:time"]/time_factor-offset, data["20:neutrinoleakage::abs_energy.sum"], s=ring_marker_size, marker="o", facecolors='none', edgecolors='b')
    else:
        plt.scatter(data["2:time"]/time_factor-offset, data["20:neutrinoleakage::abs_energy.sum"], s=dot_marker_size, marker=".", facecolors='r', edgecolors='r')
