...
def get_plasma_beta_xz(x_slice, data_dir, input_iteration, level):
    ...
    for i in range(gxx_2d_yz.shape[0]-1):
      for j in range(gxx_2d_yz.shape[1]-1):
          gxx_2d_yz_ccc[i, j] = gxx_2d_yz[i, j]
          gxy_2d_yz_ccc[i, j] = gxy_2d_yz[i, j]
          gxz_2d_yz_ccc[i, j] = gxz_2d_yz[i, j]
          gyy_2d_yz_ccc[i, j] = gyy_2d_yz[i, j]
          gyz_2d_yz_ccc[i, j] = gyz_2d_yz[i, j]
          gzz_2d_yz_ccc[i, j] = gzz_2d_yz[i, j]
          alp_2d_yz_ccc[i, j] = alp_2d_yz[i, j]
          betax_2d_yz_ccc[i, j] = betax_2d_yz[i, j]
          betay_2d_yz_ccc[i, j] = betay_2d_yz[i, j]
          betaz_2d_yz_ccc[i, j] = betaz_2d_yz[i, j]
    ...
    plasma_beta = bernoulli   #TODO TODO TODO: Remove this later on. Temporary placeholder. 
...
for output_number in range(100, 101):   #54 is the last for "Ref6_40"
    ...
    colormap = "plasma" #"seismic_r" # #"turbo"
    M_to_km = 1.477  #1.0
